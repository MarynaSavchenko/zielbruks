...
from scheduler.import_handlers import parse_data, import_csv, import_excel, ImportSizeException, \
    ImportExtensionException
...
class ImportHandlersTest(TestCase):
    ...
    def test_parse_data_extension(self):
        ...
        data = pd.DataFrame(["1", "2", "3", "4", "5", "6", "7"]).transpose()
        with self.assertRaises(ImportExtensionException):
            parse_data(data, ".txt")
        with self.assertRaises(ImportExtensionException):
            parse_data(data, ".md")
    ...
    def test_parse_data_length(self):
        ...
        with self.assertRaises(ImportSizeException):
            parse_data(too_long_data, ".csv")
        with self.assertRaises(ImportSizeException):
            parse_data(too_short_data, ".csv")
    ...
    def test_import_csv(self):
        ...
        data = pd.DataFrame(
            [["11-05-19", "12:00", "13:30", "Existing", "John Doe", "1", "1.11a"],
             ...
             ["2031-05-19", "20:00", "22:00", "Also Correct", "John Doe", "2", "2.41"],
             ["10-05-19", "13:00", "14:00", "Duplicate", "Adam Smith", "1", "132"],
             ["2019-05-10", "13:00", "14:00", "Duplicate", "Adam Smith", "1", "132"],
        ...
        self.assertEqual(incorrect, [1, 7, 8, 9, 10])
        self.assertEqual(duplicate, [0, 5, 6])
        ...
        self.assertEqual(Lesson.objects.filter(name="Also Correct").count(), 1)
